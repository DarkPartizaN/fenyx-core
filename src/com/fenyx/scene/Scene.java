...
public class Scene {
    ...
    public final SceneObject getObjectByName(String name) {
        ...
        if (this.camera != null) {
            ...
            if (this.camera.check_bounds) {
                if (this.camera.getWidth() < this.bounds.w) {
                    if (this.camera.getX() < this.bounds.x)
                        this.camera.setPosition(this.bounds.x, this.camera.getY());
                    if (this.camera.getX() + this.camera.getWidth() > this.bounds.w - this.bounds.x)
                        this.camera.setPosition(this.bounds.w - this.bounds.x - this.camera.getWidth(), this.camera.getY());
                } else {
                    if (this.camera.getX() > this.bounds.x)
                        this.camera.setPosition(this.bounds.x, this.camera.getY());
                    if (this.camera.getX() + this.camera.getWidth() < this.bounds.x + this.bounds.w)
                        this.camera.setPosition(this.bounds.x + this.bounds.w - this.camera.getWidth(), this.camera.getY());
                }
                if (this.camera.getHeight() < this.bounds.h) {
                    if (this.camera.getY() < this.bounds.y)
                        this.camera.setPosition(this.camera.getX(), this.bounds.y);
                    if (this.camera.getY() + this.camera.getHeight() > this.bounds.h - this.bounds.y)
                        this.camera.setPosition(this.camera.getX(), this.bounds.h - this.bounds.y - this.camera.getHeight());
                } else {
                    if (this.camera.getY() > this.bounds.y)
                        this.camera.setPosition(this.camera.getX(), this.bounds.y);
                    if (this.camera.getY() + this.camera.getHeight() < this.bounds.y + this.bounds.h) {
                        this.camera.setPosition(this.camera.getX(), this.bounds.y + this.bounds.h - this.camera.getHeight());
                    }
                }
            }
        }
    ...
    public final void setActor(SceneObject actor) {
        this.actor = actor;
    }

    public final SceneObject getActor() {
        return this.actor;
    }
}
